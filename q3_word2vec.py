...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    ### YOUR CODE HERE
    vc = predicted
    y_=softmax(np.dot(outputVectors,vc))
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    ### YOUR CODE HERE
    uc_indice=  tokens[currentWord]
    uc = inputVectors[uc_indice]

    for j in contextWords:
        uw_ind = tokens[j]
        cos, gradPre, gra = word2vecCostAndGradient(uc,uw_ind,outputVectors,dataset)
        cost+=cos
        gradIn[uc_indice] += gradPre
        gradOut+=gra
    ### END YOUR CODE
